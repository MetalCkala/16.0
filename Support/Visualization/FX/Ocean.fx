...
float gTransparency <
    string UIName = "Water transparency";
    string UIWidget = "slider";    
    float UIMin = 0.0;
    float UIMax = 1.0;
    float UIStep = 0.01;    
...
OceanVertOut OceanVS(AppData IN,
    ...
    {
		Po.z += evaluateWave(wave[i], Po.xy, Timer);
		float deriv = evaluateWaveDeriv(wave[i], Po.xy, Timer);
    ...
    OUT.UV = IN.Position.xy * TextureScale;
    float cycle = fmod(Timer, 100.0);
    OUT.bumpUV0.xy = OUT.UV + cycle * BumpSpeed;
    OUT.bumpUV1.xy = OUT.UV * 2.0 + cycle * BumpSpeed * 4.0;
    OUT.bumpUV2.xy = OUT.UV * 4.0 + cycle * BumpSpeed* 8.0;
    ...
    OUT.WorldView = ViewIXf[3].xzy - Pw.xzy; // view inv. transpose contains eye position in world space in last row
    return OUT;
}
...
) : COLOR
{
    ...
    return float4(result.rgb, Transparency);
}
...
> {
    ...
    > {
        ...
        SetPixelShader( CompileShader( ps_4_0, OceanPS(gEnvSampler,
		    gKr,gKWater,gFresnelExp,gFresnelBias,gHDRMultiplier, gTransparency,
